...
Aplicacion::Aplicacion(HINSTANCE pinstancia) : instancia(pinstancia), ventana(NULL) {}

// Crea la ventana y ejecuta el bucle principal de mensajes.
int Aplicacion::ejecutar(int mostrar) {
    registrarClase();
    crearVentana();
...
LRESULT Aplicacion::manejarMensaje(HWND hwnd, UINT mensaje, WPARAM wParam, LPARAM lParam) {
    switch (mensaje) {
        case WM_CREATE:
...
void Aplicacion::registrarClase() {
    WNDCLASSW wc;
...
void Aplicacion::dibujar(HWND hwnd) {
    PAINTSTRUCT ps;
...
// ========================================================================
int Aplicacion::convertirX(LPARAM lParam, HWND hwnd) const {
...
int Aplicacion::convertirY(LPARAM lParam, HWND hwnd) const {
    EscalaVentana escala = EscalaVentana::desdeVentana(hwnd);
    if (escala.alto == 0) {
        return 0;
    }
...
// ========================================================================
LRESULT CALLBACK Aplicacion::procedimientoVentana(HWND hwnd, UINT mensaje, WPARAM wParam, LPARAM lParam) {
    Aplicacion* app = NULL;
